...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, pos, enemy_type='goomba'):
        ...
        self.update_image()  # Draw initial appearance

    def update_image(self):
        """Redraw the enemy using the current theme color."""
        self.image.fill(themes[current_theme]['enemy'])
        # Draw enemy details based on type (these details are not theme-dependent)
...
class PowerUp(pygame.sprite.Sprite):
    ...
    def update_image(self):
        ...
        elif self.powerup_type == 'fire_flower':
...
def set_theme(theme_name):
    global current_theme, tile_icons
    if theme_name in themes:
        ...
        for sprite in all_sprites:
            if isinstance(sprite, Tile):
                ...
                sprite.update_image()  # Redraw powerup (some may use theme colors)
        # Rebuild HUD icons with new theme
        rebuild_tile_icons()
        print(f"Theme set to '{theme_name}'.")
    else:
        print(f"Theme '{theme_name}' does not exist.")

def rebuild_tile_icons():
    global tile_icons
    tile_icons = {}
...
class AddAction(Action):
    def __init__(self, pos, tile_type, **kwargs):
        self.pos = pos
        self.tile_type = tile_type
        self.kwargs = kwargs  # for enemy_type, powerup_type, contains_item, value, etc.
    ...
    def redo(self):
        if self.tile_type in ['ground', 'brick', 'question', 'pipe', 'water', 'platform']:
            ...
            if 'contains_item' in self.kwargs:
                tile.contains_item = self.kwargs['contains_item']
            ...
            enemy_type = self.kwargs.get('enemy_type', 'goomba')
            ...
            if 'value' in self.kwargs:
                coin.value = self.kwargs['value']
            coins_group.add(coin)
            all_sprites.add(coin)
        elif self.tile_type == 'powerup':
            powerup_type = self.kwargs.get('powerup_type', 'mushroom')
...
class RemoveAction(Action):
    def __init__(self, sprite):
        ...
        if isinstance(sprite, Tile):
            self.sprite_info['tile_type'] = sprite.tile_type
            if hasattr(sprite, 'contains_item'):
                self.sprite_info['contains_item'] = sprite.contains_item
        elif isinstance(sprite, Enemy):
            self.sprite_info['tile_type'] = 'enemy'
            self.sprite_info['enemy_type'] = sprite.enemy_type
        elif isinstance(sprite, Coin):
            self.sprite_info['tile_type'] = 'coin'
            self.sprite_info['value'] = sprite.value
        elif isinstance(sprite, PowerUp):
            self.sprite_info['tile_type'] = 'powerup'
            self.sprite_info['powerup_type'] = sprite.powerup_type

    def undo(self):
        info = self.sprite_info
        pos = info['pos']
        tile_type = info['tile_type']
        if tile_type in ['ground', 'brick', 'question', 'pipe', 'water', 'platform']:
            tile = Tile(pos, tile_type)
            if info['contains_item']:
                tile.contains_item = info['contains_item']
            ...
            enemy = Enemy(pos, info['enemy_type'])
            enemies_group.add(enemy)
            all_sprites.add(enemy)
        elif tile_type == 'coin':
            coin = Coin(pos)
            coin.value = info['value']
            coins_group.add(coin)
            all_sprites.add(coin)
        elif tile_type == 'powerup':
            powerup = PowerUp(pos, info['powerup_type'])
            powerups_group.add(powerup)
            all_sprites.add(powerup)

    def redo(self):
        for sprite in all_sprites:
            if sprite.rect.topleft == self.sprite_info['pos'] and sprite != player:
                sprite.kill()
                break
...
tile_icons = {}
rebuild_tile_icons()  # Initial build
                        ...
                        if event.button == 1:  # Left click to add
                            ...
                            for sprite in all_sprites:
                                if sprite.rect.topleft == grid_pos and sprite != player:
                                    actions.append(RemoveAction(sprite))
                                    sprite.kill()
                                    break
                            # Add new sprite
                            kwargs = {}
                            if selected_tile_type == 'enemy':
                                kwargs['enemy_type'] = 'goomba'  # default, could be selected from UI later
                            elif selected_tile_type == 'powerup':
                                kwargs['powerup_type'] = 'mushroom'
                            actions.append(AddAction(grid_pos, selected_tile_type, **kwargs))
                            
                            composite = CompositeAction(actions)
                            undo_stack.append(composite)
                            redo_stack.clear()
                            composite.redo()  # Perform the add
