"""
SMBX 1.3 Style Level Editor (Bug Fixed Implementation)
Fixes:
1. NPCs now Save and Load correctly.
2. NPC Placement Undo crash fixed (NameError).
3. Drag painting/erasing implemented.
4. NPC Removal Undo/Redo implemented.
"""
...
class Sidebar:
    ...
    def draw_items(self, surf):
        ...
        for i, item in enumerate(items):
            ...
            # Improved color mapping for sidebar icons
...
class Editor:
    def __init__(self, level):
        ...
        self.setup_toolbar()
        self.drag_draw = False # Track if we are dragging to draw
        self.drag_erase = False # Track if we are dragging to erase
    ...
    def save_level(self):
        ...
        for sec in self.level.sections:
            ...
            for lay in sec.layers:
                lay_data = {"name": lay.name, "tiles": [], "npcs": []} # Added npcs list
                for t in lay.tiles:
        ...
        try:
            with open("level.json", "w") as f: json.dump(data, f)
            print("Level saved to level.json (Includes NPCs)")
        except Exception as e: print(f"Error saving: {e}")

    def load_level(self):
        try:
            with open("level.json", "r") as f: data = json.load(f)
    ...
    def place_tile(self, x, y):
        ...
        if self.sidebar.selected_item in ['goomba', 'koopa', 'mushroom']:
            ...
            layer.npcs.add(npc)
            # FIX 2: Use 'layer' instead of undefined 'l'
        ...
        if key in layer.tile_map:
            ...
            for npc in layer.npcs:
                if npc.rect.collidepoint(x, y):
                    ...
                    # FIX 4: Implement NPC removal Undo/Redo
    ...
    def handle_event(self, event):
        ...
        # FIX 3: Handle Mouse Drag for drawing/erasing
...
def main_menu(screen):
    buttons = [
        Button((WINDOW_WIDTH//2 - 100, 300, 200, 50), "New Level", lambda: "NEW"),
        Button((WINDOW_WIDTH//2 - 100, 370, 200, 50), "Quit", lambda: "QUIT")
    ]
    while True:
        ...
        screen.fill(BLACK)
        pygame.draw.rect(screen, BLUE, (0, 100, WINDOW_WIDTH, 200))
        draw_text(screen, "SMBX PYTHON EDITION", (WINDOW_WIDTH//2, 150), WHITE, FONT_TITLE, True)
        draw_text(screen, "Fixes: Save/Load NPCs, Drag Draw, Undo Crashes", (WINDOW_WIDTH//2, 200), LIGHT_GRAY, FONT, True)
...
def main():
    ...
    pygame.display.set_caption("SMBX Python Editor (Bug Fixed)")
    clock = pygame.time.Clock()
    while True:
        result = main_menu(screen)
        if result == "QUIT": pygame.quit(); sys.exit()
        if result == "NEW":
            level = Level(); editor = Editor(level)
            running = True
            while running:
                for event in pygame.event.get():
                ...
                editor.update(); editor.draw(screen)
                pygame.display.flip()
                clock.tick(FPS)
